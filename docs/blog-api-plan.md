...
**검색(`GET /posts/search`)도 raw 쿼리** — 처음엔 `to_tsvector` + `setweight` + `ts_rank`로 설계했으나, 실제 Neon DB로 검증하다가 **한글 부분검색이 근본적으로 안 되는 문제를 발견**: `to_tsvector`는 한글을 형태소가 아니라 공백 기준 어절 단위로 토큰화해서(Neon엔 한글 사전 확장이 없음), "JWT 인증과 Prisma"라는 제목에서 "인증"으로 검색하면 매칭 안 됨(`인증과`가 하나의 토큰이라) — 직접 쿼리로 `false` 확인. `pg_trgm`(trigram 유사도)도 대안으로 테스트했지만 짧은 검색어 대비 긴 제목의 유사도 점수가 기본 임계값(0.3)보다 한참 낮아(`0.047`) 튜닝 없인 매칭 자체가 안 됨. 블로그 콘텐츠가 대부분 한글이라 이 문제를 무시할 수 없어서, **ILIKE 부분문자열 매칭 + 수동 가중치 정렬**로 설계 변경(사용자와 논의 후 결정): `title ILIKE '%q%'`(3점) `> tags 배열에 포함`(2점) `> body ILIKE '%q%'`(1점) 순으로 `CASE WHEN` 점수를 매겨 `ORDER BY`. 개인 블로그 규모에서 인덱스 없는 `ILIKE`/`CASE WHEN` 정렬은 성능 문제 없음(글 수가 수천 단위로 늘면 그때 `pg_trgm` GIN 인덱스 등을 재검토 — YAGNI). `tags` 배열 파라미터는 `pg`(Prisma 7 adapter의 기반 드라이버) 바인딩으로 JS 배열이 `text[]`로 그대로 넘어가 `@>` 연산이 되는 것도 실제 쿼리로 확인.
...
10. [x] R2 연동 — 이미지 업로드 API — `@aws-sdk/client-s3`로 `R2Service`(`region: 'auto'`, `endpoint: R2_ENDPOINT`, Access Key/Secret) 구성(공식 Cloudflare R2 S3 SDK 예제로 확인), `ImageController`(`POST /images`, `JwtGuard`+`RolesGuard('OWNER')`)가 `FileInterceptor('file')`(multer 기본 memoryStorage라 `file.buffer` 바로 사용, 별도 옵션 불필요 — multer 소스로 확인)로 받아 `ParseFilePipe`(`MaxFileSizeValidator` 5MB, `FileTypeValidator` jpeg/png/webp/gif)로 검증 후 R2에 `images/{uuid}.{ext}`로 업로드, 공개 URL(`R2_PUBLIC_URL_BASE/{key}`) 반환. 확장자는 클라이언트가 보낸 파일명이 아니라 검증된 mimetype에서 파생(파일명 신뢰 안 함). 버킷은 `blog-assets`(Standard 클래스, r2.dev Public Development URL로 공개 — 프로덕션 트래픽 늘면 커스텀 도메인으로 전환 검토, 버킷 재생성은 불필요). **구현 중 잡은 버그**: `ImageModule`이 `AuthModule`을 import 안 해서 `JwtGuard`의 `JwtService` 의존성 주입 실패로 부팅 자체가 안 됐음 — `PostModule` 등 다른 모듈과 동일하게 `AuthModule` import 추가. 실제 Cloudflare R2로 검증(프로덕션 빌드): 정상 업로드 후 반환된 URL이 실제로 공개 접근 가능(200, `content-type: image/png`)한 것까지 확인, 무인증(401), 5MB 초과 파일(400), 이미지가 아닌 MIME(400) 거부 확인. 테스트로 올린 객체는 검증 후 R2에서 직접 삭제(리소스 안 남김)
11. [x] 검색(`$queryRaw` 가중치 랭킹) + 조회수/좋아요 + 통계 API
    - `GET /posts/search`(`q` 필수, `sort=latest|relevance` 기본 relevance, `category`/`tags` 필터) — 위 "API 엔드포인트" 절 설명대로 `to_tsvector`가 한글에서 안 먹히는 걸 실제 Neon으로 발견하고 ILIKE + 수동 가중치(`title` 3 > `tags` 2 > `body` 1) 정렬로 설계 변경 후 구현. `PostController`에서 `@Get('search')`를 `@Get(':slug')`보다 먼저 선언(안 그러면 "search"가 slug로 잡혀버림). `OptionalJwtGuard`로 OWNER는 hidden 글도 포함(`GET /posts`와 동일 패턴). `tags` 배열 파라미터를 `pg`(Prisma 7 adapter 기반 드라이버)에 그대로 바인딩해 `@>` 연산이 되는 것도 실제 쿼리로 확인
